function [tau_f tau_r] = function(omega_fk, omega_rk, Vref, state_param, first_d, pHorizon, dt, r, I_wf, I_wr)
    ...
    next_lng_pos_f = omega_fk*r*dt;
    next_lng_pos_r = omega_rk*r*dt;
    ...
    next_d(3) = round(makima(current_dis,current_wheel_traj_f,next_d(1)-first_d(1))*1e05)*1e-05;
    next_d(4) = round(makima(current_dis,current_wheel_traj_r,next_d(2)-first_d(2))*1e05)*1e-05;

    duration_length_f = dt*Vref*r/2;
    ...
    temp_gradient_f = [sum(diff(temp_duration_f)./(dt/2))/2;sum(diff(temp_displacment_f)./(dt/2))/2];
    duration_length_r = dt*Vref*r/2;
    ...
    temp_gradient_r = [sum(diff(temp_duration_r)./(dt/2))/2;sum(diff(temp_displacment_r)./(dt/2))/2];
