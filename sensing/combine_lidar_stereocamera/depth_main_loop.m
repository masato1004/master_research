...
while (key ~= 27)
    ...
    if(strcmp(result,'SUCCESS'))  % get stereo camera data
        ...
        pcwrite(zedCloud,savedir_zed+count+"-"+datestr(imutimestamp,"HH-mm-ss_FFF"),PLYformat="binary")

        % get imu data
        sensors_data = mexZED('getSensorsData', 1); % ask CURRENT sensors data
    end

    pcwrite(lidarCloud,"OusterLiDARply/"+exp_purpose+"/"+count+"-"+datestr(lidartimestamp,"HH-mm-ss_FFF"),PLYformat="binary")  % save lidar data as ply
