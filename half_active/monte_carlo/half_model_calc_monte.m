...
run("conf__simulation_conditions.m")
    ...
    if mod(i+(ts/dt-1), ts/dt) == 0
        ...
        if sensing
            if sc <= height(listing)
                file = load("../sim_config_mfiles/"+load_dir + "/" + listing(sc).name);
        ...
        if low_freq_noise
            lnoise = lsd(tan(plane_errors_set(loop,s_count,1)),plane_errors_set(loop,s_count,2),wf_local(1,:));
        ...
        if wa
            ...
            if sensing
                ...
                wf_global = [
                    wf_global(1, wf_global(1,:)<wf_local(1,1) & wf_global(1,:)>-200), wf_local(1,:);
                    wf_global(2, wf_global(1,:)<wf_local(1,1) & wf_global(1,:)>-200), makima(wf_global(1,:), wf_global(2,:), wf_local(1, wf_local(1,:)<=wf_global(1,end))).*eta_1 + wf_local(2, wf_local(1,:)<=wf_global(1,end)).*eta_2, wf_local(2, wf_local(1,:)>wf_global(1,end))
                    ];
            end
