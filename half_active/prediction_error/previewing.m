function [prev_profile, angle_d, predicted_height] = previewing(vertices)
...
%     % mesh2 = mesh(X_pre,Y_pre,Z_pre,"AlphaData",0.5,"EdgeColor","g","FaceColor","none");
    ...
    %% PCFITPLANE
    mx_er_dis = 0.02;  % [m]
    model = pcfitplane(ptCloudA,mx_er_dis);
    new_ver = ptCloudA.Location;
    X_max = max(new_ver(:,1),[],'all');
    X_min = min(new_ver(:,1),[],'all');
    Y_max = max(new_ver(:,2),[],'all');
...
%     mesh2 = mesh(X_pre,Y_pre,Z_pre,"AlphaData",0.5,"EdgeColor","r","FaceColor","none");
    ...
    angle_r = subspace(z_vec',u_vec');          % [rad] surface angle
    angle_d = rad2deg(angle_r);                 % [deg] surface angle
    ...
    predicted_height = -mean(new_mesh_out.Location(:,3));
    ptc_datas(:,3) = ptc_datas(:,3) + predicted_height;
    % figure;
