...
ssSetZcSignalWidth ( S , zcsIdx , 4 ) ; ssSetZcSignalName ( S , zcsIdx ,
...
ssCreateAndAddZcSignalInfo ( S ) ; ssSetZcSignalWidth ( S , zcsIdx , 4 ) ;
...
zcsIdx , 1 ) ; ssSetZcSignalName ( S , zcsIdx , "x Upper Saturation" ) ;
ssSetZcSignalType ( S , zcsIdx , SL_ZCS_TYPE_CONT ) ;
ssSetZcSignalZcEventType ( S , zcsIdx , SL_ZCS_EVENT_ALL_UP ) ;
ssSetZcSignalNeedsEventNotification ( S , zcsIdx , 0 ) ; zcsIdx =
ssCreateAndAddZcSignalInfo ( S ) ; ssSetZcSignalWidth ( S , zcsIdx , 1 ) ;
ssSetZcSignalName ( S , zcsIdx , "x Lower Saturation" ) ; ssSetZcSignalType ( S
, zcsIdx , SL_ZCS_TYPE_CONT ) ; ssSetZcSignalZcEventType ( S , zcsIdx ,
SL_ZCS_EVENT_ALL_DN ) ; ssSetZcSignalNeedsEventNotification ( S , zcsIdx , 0
) ; zcsIdx = ssCreateAndAddZcSignalInfo ( S ) ; ssSetZcSignalWidth ( S ,
zcsIdx , 1 ) ; ssSetZcSignalName ( S , zcsIdx , "Input Port Signal" ) ;
ssSetZcSignalType ( S , zcsIdx , SL_ZCS_TYPE_CONT ) ;
ssSetZcSignalZcEventType ( S , zcsIdx , SL_ZCS_EVENT_ALL ) ;
...
#define MDL_UPDATE
static void mdlUpdate ( SimStruct * S , int_T tid ) { egcgcay4tuh * dw = ( egcgcay4tuh * ) ssGetDWork ( S , 0 ) ; gy0s4k0fxb * localXdis = ( gy0s4k0fxb * ) ssGetContStateDisabled ( S ) ; mq10j5tdpd * localXAbsTollocalXAbsTol = ( mq10j5tdpd * ) ssGetAbsTolVector ( S ) ; jpl1qt0btr ( & ( dw -> rtm ) , & ( dw -> rtb ) , & ( dw -> rtdw ) , localXdis , localXAbsTollocalXAbsTol ) ; return ; }
#define MDL_ZERO_CROSSINGS
static void mdlZeroCrossings ( SimStruct * S ) { egcgcay4tuh * dw = ( egcgcay4tuh * ) ssGetDWork ( S , 0 ) ; h21fsrthfa * localX = ( h21fsrthfa * ) ssGetContStates ( S ) ; beahf2xhqn * localZCSV = ( beahf2xhqn * ) ssGetNonsampledZCs ( S ) ; gahgwqe10m ( & ( dw -> rtb ) , & ( dw -> rtdw ) , localX , localZCSV ) ; }
...
egcgcay4tuh * dw = ( egcgcay4tuh * ) ssGetDWork ( S , 0 ) ; apei0jh2jv ( & ( dw
-> rtm ) ) ; return ; }
...
, 2 , data ) ; } ; mxSetFieldByNumber ( ss , 0 , 3 , mxCreateDoubleScalar ( ( double ) ssGetTNext ( S ) ) ) ; mxSetFieldByNumber ( ss , 0 , 4 , mxCreateDoubleScalar ( ( double ) ssGetTNextTid ( S ) ) ) ; { egcgcay4tuh * dw = ( egcgcay4tuh * ) ssGetDWork ( S , 0 ) ; gwlxzditat * rtm = & ( dw -> rtm ) ; mxArray * nonContDerivSigInfo = mxCreateCellMatrix ( 120 , 1 ) ; for ( int i = 0 ; i < 120 ; ++ i ) { mxArray * prevValmxArray = mxCreateNumericMatrix ( 1 , rtm -> nonContDerivSignal [ i ] . sizeInBytes , mxUINT8_CLASS , mxREAL ) ; memcpy ( ( uint8_T * ) mxGetData ( prevValmxArray ) , ( const uint8_T * ) rtm -> nonContDerivSignal [ i ] . pPrevVal , rtm -> nonContDerivSignal [ i ] . sizeInBytes ) ; mxSetCell ( nonContDerivSigInfo , i , prevValmxArray ) ; } mxSetFieldByNumber ( ss , 0 , 5 , nonContDerivSigInfo ) ; } return ss ; }
#if !defined(MDL_SIM_STATE)
#define MDL_SIM_STATE
#endif
...
, 0 , 5 ) ; egcgcay4tuh * dw = ( egcgcay4tuh * ) ssGetDWork ( S , 0 ) ;
gwlxzditat * rtm = & ( dw -> rtm ) ; for ( int i = 0 ; i < 120 ; ++ i ) {
