...
accelerations = [
    out.Body_bdy.Cg.Acc.ax.Data';
    out.Body_bdy.Cg.Acc.ay.Data';
    out.Body_bdy.Cg.Acc.az.Data'
    ];
...
for i = 1:double(temp.DrawFreq/0.001):length(positions)
    ...
    time = out.tout(i);
    [frt,frt_l,frt_r] = XYplot(time,pos,ang,vel,acc,wheel_ang,temp.T_ref,temp);
    front = [front; frt];
...
function [front,front_left,front_right] = XYplot(time,pos,agl,vel,acc,wheel_agl,T_ref,temp)
...
bdy_acc = [acc_y; acc_x].*10;
bdy_acc_y = [acc_y; 0].*10;
bdy_acc = R_bdy*bdy_acc;
bdy_acc_y = R_bdy*bdy_acc_y;
...
txt_time = time+" s";
persistent wfl_p wfr_p wrl_p wrr_p bdy_p vel_p time_p cg_sc axl_sc acc_p accy_p
if time<0.000001
    ...
    acc_p = quiver(body_y,body_x,bdy_acc(1),bdy_acc(2),0,"Color",'green','LineWidth',2.5,'ShowArrowHead','on');
    accy_p = quiver(body_y,body_x,bdy_acc_y(1),bdy_acc_y(2),0,'r','LineWidth',2.5,'ShowArrowHead','on');
    ...
    set(acc_p,'XData',body_y,'YData',body_x,'UData',bdy_acc(1),'VData',bdy_acc(2));
    set(accy_p,'XData',body_y,'YData',body_x,'UData',bdy_acc_y(1),'VData',bdy_acc_y(2));
