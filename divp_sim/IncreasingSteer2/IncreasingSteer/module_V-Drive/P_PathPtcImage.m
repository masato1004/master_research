...
pcd_dir_name = "pcd_"+lidar_name+"-lidar";
img_dir_name = "image_front";
...
filename = "scenario/dataset_1/scenario_1_divp_Veh_NissanXtrail_1.csv";
...
for i = 1:1:imax
    ...
    if i > 1
        ...
        if use_ptc
            ...
            [depth,lidar_camerapoints] = func_projectLidarToDepthImage(ptCloud, eye(4), focalLength, principalPoint, imageSize, RadialDistortion6, TangentialDistortion);
        end
        ...
        if use_ptc && ~use_img
            % view1
            subplot(2,2,1)
            color = ptCloud.Location(:,3);
            if length(ptCloud.Intensity) ~= 0
                color = ptCloud.Intensity;
            end
            lidar = scatter3(ptCloud.Location(:,1)+camera_position(1),ptCloud.Location(:,2),ptCloud.Location(:,3)+camera_position(3),2,double(color),'fill');
            hold on
