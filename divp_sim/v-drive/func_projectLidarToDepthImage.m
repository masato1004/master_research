function [depth_for_img,cameraPoints] = func_projectLidarToDepthImage(pcd, tmat_cam2lidar, fl, pp, ims, rd, td)
    ...
    r2 = (sqrt(r2) .* radFact).^2;

    normP(:, 1)= normP(:, 1) .* radFact;
    normP(:, 2)= normP(:, 2) .* radFact;
    
    xDistorted = normP(:, 1) + 2 * td(1) * normP(:, 1) .* normP(:, 2) + td(2) * (r2 + 2 * normP(:, 1).^2);
    yDistorted = normP(:, 2) + td(1) * (r2 + 2 * normP(:, 2).^2) + 2 * td(2) * normP(:, 1) .* normP(:, 2);
